...
def test_stream_callback_song_basic_fields() -> None:
    s = StreamCallbackSong.model_validate({
        "artist": "X", "title": "Y", "score": 100,
        "isrc": "USRC12345678", "upc": "0123456789012",
    })
    assert s.artist == "X"
    assert s.score == 100
    assert s.isrc == "USRC12345678"
    assert s.upc == "0123456789012"


def test_stream_callback_match_flat_shape() -> None:
    m = StreamCallbackMatch.model_validate({
        ...
        "alternatives": [{"artist": "X2", "title": "Y2", "score": 80}],
    })
    assert m.radio_id == 7
    assert m.song.artist == "X"
    assert m.play_length == 111
    assert len(m.alternatives) == 1
    assert m.alternatives[0].title == "Y2"


def test_stream_callback_notification_with_time() -> None:
    n = StreamCallbackNotification.model_validate({
        "radio_id": 3, "stream_running": False,
        "notification_code": 650, "notification_message": "can't connect",
        "time": 1587939136,
    })
    assert n.notification_code == 650
    assert n.time == 1587939136
    assert n.stream_running is False
