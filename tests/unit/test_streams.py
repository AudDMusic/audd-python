...
@respx.mock
def test_set_callback_url_extra_parameters_propagated() -> None:
    ...
    AudD(api_token="t").streams.set_callback_url(
        "https://x.com/cb",
        extra_parameters={"foo": "bar", "url": "should-not-overwrite"},
    )
    assert "foo=bar" in captured["body"]
    # Typed url wins over extras["url"] on collision.
    assert "url=https%3A%2F%2Fx.com%2Fcb" in captured["body"]
    assert "should-not-overwrite" not in captured["body"]


@respx.mock
def test_add_stream_extra_parameters_propagated() -> None:
    captured: dict[str, str] = {}
    ...
    respx.post("https://api.audd.io/addStream/").mock(side_effect=handler)
    AudD(api_token="t").streams.add(
        "https://stream.url",
        radio_id=1,
        extra_parameters={"foo": "bar", "radio_id": "999"},
    )
    assert "foo=bar" in captured["body"]
    # Typed radio_id wins over extras["radio_id"] on collision.
    assert "radio_id=1" in captured["body"]
    assert "radio_id=999" not in captured["body"]
...
def test_derive_longpoll_category_helper() -> None:
    cat = AudD(api_token="0123456789abcdef0123456789abcdef").streams.derive_longpoll_category(1)
    assert cat == "e31a31e76"
