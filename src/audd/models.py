...
class StreamCallbackMatch(_Forward):
    ...
    The top match is always available as ``song``. Rare extra candidates —
    which **may have a different artist or title** from the top match (variant
    catalog releases, near-duplicates) — live in ``alternatives``.

    The full unparsed callback body is available as ``raw_response`` for
    debugging or forwarding; unknown top-level keys round-trip through
