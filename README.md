...
Official Python SDK for [AudD](https://audd.io) — music recognition from a short audio clip, a long audio file, or a live stream.
...
```python
# In your webhook handler, given the parsed JSON `body`:
payload = audd.streams.parse_callback(body)
if payload.is_result:
    ...
    print("notification:", payload.notification.notification_message)
```
...
for event in audd.streams.longpoll(category, timeout=30):
    print(event)  # raw dict — same shape as a callback payload
```
...
> For recognition, use `recognize()` (or `recognize_enterprise()` for files longer than 25 seconds). The custom-catalog endpoint adds songs to your *private* fingerprint database so future `recognize()` calls on your account can identify *your own* tracks.
